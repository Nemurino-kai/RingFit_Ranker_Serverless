...
def search_exercise_data(api, max_number=300,interrupt=True):
    ...
    for tweet in tweepy.Cursor(api.search, q=f'#リングフィットアドベンチャー -filter:retweets filter:images',tweet_mode="extended").items(max_number):
        ...
        cur.execute("select count(*) from Exercise where tweet_id = %s", (tweet.id,))
...
def reply_ranking(api,item_num=100):
    for tweet in tweepy.Cursor(api.search, q=f'@{TWITTER_ID}',tweet_mode="extended").items(item_num):
        ...
        # user_idからカロリーを抽出
        cur.execute("SELECT kcal FROM Exercise WHERE user_id == %s AND  CAST(tweeted_time - INTERVAL 4 HOUR AS DATE)=CAST(%s AS DATE)",params)
...
def tweet_ranking(api):
    ...
    cur.execute("SELECT user_name,kcal "
                "FROM (SELECT *, RANK() OVER(PARTITION BY user_screen_name ORDER BY kcal DESC, tweeted_time ASC) AS rnk FROM Exercise WHERE CAST(tweeted_time - INTERVAL 4 HOUR AS DATE)=CAST(%s AS DATE)) tmp "
...
def reply_exercise_result(api,cur,exercise_data,status):
    ...
    cur.execute("select kcal from Exercise "
                "WHERE  CAST(tweeted_time - INTERVAL 4 HOUR AS DATE)=CAST(%s AS DATE) ORDER BY kcal DESC ;",params)
    exercise_data_list = cur.fetchall()
    print(exercise_data)

    # 消費カロリーの順位を計算する
    params = (exercise_data.cal,ranking_datetime)
    cur.execute("select count(*) from Exercise WHERE Exercise.kcal > %s "
                "AND  CAST(tweeted_time - INTERVAL 4 HOUR AS DATE)=CAST(%s AS DATE)", params)
